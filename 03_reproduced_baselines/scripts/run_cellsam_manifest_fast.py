...
def _read_image(path: Path, grayscale_mode: str) -> np.ndarray:
    ...
    if arr.ndim == 2:
        ...
        out[:, :, {"red": 0, "green": 1, "blue": 2}[grayscale_mode]] = ch
        return out
    if arr.ndim != 3:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run official CellSAM inference with one persistent model over a manifest.")
    ...
    parser.add_argument("--run_manifest_name", default="run_manifest.json")
    args = parser.parse_args()

    from cellSAM.cellsam_pipeline import normalize_image, use_cellsize_gaging
    ...
    started = time.time()
    if args.model_path:
        model = get_local_model(args.model_path)
        model.bbox_threshold = float(args.bbox_threshold)
    else:
        model = get_model()
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        print(f"[{idx}/{len(rows)}] CellSAM-fast {image_path.name}", flush=True)
        if args.skip_existing and pred_path.exists():
            ...
            continue
        image = _read_image(image_path, args.grayscale_mode).astype(np.float32)
        ...
        if args.low_contrast_enhancement:
            image = enhance_low_contrast(image)
        inp = da.from_array(image, chunks=int(args.chunks))
        status = "done"
        with torch.inference_mode():
            try:
                if args.use_wsi:
                    if args.gauge_cell_size:
    ...
    (out_dir / args.run_manifest_name).write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
