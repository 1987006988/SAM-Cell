...
def _row_name(row: dict[str, str]) -> str:
    for key in ["image", "image_name", "Image_Name", "image_path", "Image_Path"]:
        value = row.get(key)
        if value:
...
def _records_from_roots(image_root: Path, mask_root: Path, excluded: set[str]) -> list[dict[str, str]]:
...
def _records_from_manifest(path: Path, excluded: set[str]) -> list[dict[str, str]]:
    records = []
    for row in _read_rows(path):
        ...
        if image_name in excluded:
            continue
        source = row.get("source") or _source_from_name(image_name)
...
def _sample_records(records: list[dict[str, str]], quotas: dict[str, int], seed: int) -> list[dict[str, str]]:
    ...
    for record in records:
        ...
        if source not in quotas or quotas[source] <= 0:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a Cellpose-style boundary/interior nnU-Net dataset.")
    ...
    parser.add_argument("--manifest_csv", help="Optional manifest with source,image_path,mask_path columns.")
    parser.add_argument("--nnunet_raw", default="/home/taotao/nnUNet/nnUNetFrame/nnUNet_raw")
    parser.add_argument("--dataset_id", type=int, default=622)
    parser.add_argument("--dataset_name", default="SAMCellCellposeStyleBoundary")
    ...
    parser.add_argument("--source_quota", nargs="*", default=["cellpose=100000"])
    ...
    if args.manifest_csv:
        records = _records_from_manifest(Path(args.manifest_csv), excluded)
    else:
        records = _records_from_roots(Path(args.image_root), Path(args.mask_root), excluded)
    source_available = Counter(r["source"] for r in records)
    records = _sample_records(records, quotas, args.seed)
    if not records:
        raise ValueError("No records selected. Check paths, source_quota, and exclude_csv.")

    mapping_rows = []
    for idx, record in enumerate(records):
        source = _safe_id(record["source"])
        case_id = f"cellposestyle_{idx:04d}_{source}"
        image_out = images_tr / f"{case_id}_0000.png"
        label_out = labels_tr / f"{case_id}.png"
        _copy_image(Path(record["image_path"]), image_out)
    ...
    with (dataset_dir / "case_mapping.csv").open("w", encoding="utf-8", newline="") as f:
        fieldnames = ["case_id", "source", "image_name", "image_path", "mask_path"]
    ...
    summary = {
        ...
        "source_available": dict(source_available),
        "source_written": dict(Counter(row["source"] for row in mapping_rows)),
        "source_quota": quotas,
        "boundary_radius": args.boundary_radius,
    }
    (dataset_dir / "build_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
