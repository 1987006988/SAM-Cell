...
WAIT_SESSION="${WAIT_SESSION:-full_cellsam_prestart}"
...
LOG="$LOG_DIR/start_extra_samcell_refine_after_cellsam_main_done.log"
...
cd "$PROJECT"
date
echo "waiting for $WAIT_SESSION to finish before starting extra SAM-Cell refine helper"
echo "target session: $SESSION_NAME"
echo "target GPU: $CUDA_VISIBLE_DEVICES"

while session_alive "$WAIT_SESSION"; do
  ...
  echo "$(date '+%F %T %Z') $WAIT_SESSION still active; cellsam=$cellsam_labels/16777 samcell_refine=$samcell_labels/16777"
  sleep "$POLL_SECONDS"
done
