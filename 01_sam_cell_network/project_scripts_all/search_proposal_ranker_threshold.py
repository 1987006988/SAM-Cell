...
def _comparison_rows(sam_rows: list[dict], cp_metrics: dict[str, dict[str, float | int]], threshold: float) -> list[dict]:
    ...
    for row in sam_rows:
        ...
        output = {
            ...
            "source": row["source"],
            "image": row["image"],
            "universal_boundary_selected": int(row.get("universal_boundary_selected", 0)),
            "cellpose_style_selected": int(row.get("cellpose_style_selected", 0)),
            "ranked_proposals": int(row.get("ranked_proposals", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Search proposal ranker thresholds using instance metrics.")
    parser.add_argument("--config", default="configs/sam_cell_multi_expert_dual_ranked.yaml")
    parser.add_argument("--devset_csv", default="outputs/benchmark_splits_large/eval_25_balanced.csv")
    parser.add_argument("--cellpose_dir", default="outputs/benchmark_splits_large/cellpose_cyto")
    parser.add_argument("--out_dir", default="outputs/proposal_ranker_threshold_search")
    parser.add_argument("--thresholds", default="0.45,0.55,0.65,0.75")
    ...
    for threshold in _parse_thresholds(args.thresholds):
        ...
        pipeline.cfg.proposal_ranker.keep_threshold = threshold
